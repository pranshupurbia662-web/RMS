...
function TableSelection() {
  ...
  // ✅ Duplicate hataya — sirf ek sahi wala rakha
  const handleSelectTable = async (table) => {
    try {
      if (table.status === "Available") {
        await axios.patch(
          `http://localhost:5000/api/tables/${table._id}/status`,
          { status: "Occupied" }
        );
      }
  ...
  const getTableStyle = (status) => {
    if (status === "Available") return "bg-green-50 border-green-300 text-green-900";
    if (status === "Occupied") return "bg-red-50 border-red-300 text-red-800";
      ...
      <div className="max-w-6xl mx-auto mt-10 px-4">
        <div className="bg-white rounded-[28px] border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="bg-[#D4A017] px-8 py-6">
            <h1 className="text-white text-3xl font-bold">Table Selection</h1>
            <p className="text-yellow-100 mt-1">Select a table to start order</p>
          </div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
                {tables.map((table) => (
                  ...
                  >
                    <h2 className="font-bold text-xl mb-1">Table {table.tableNo}</h2>
                    <p className="text-sm mb-4">{table.seats} Seats</p>
                    ...
                    <button
                      ...
                      `}
                    >
                      {table.status === "Occupied" ? "Add Items" : "Select Table"}
                    </button>
                  </div>
                ))}
              </div>
