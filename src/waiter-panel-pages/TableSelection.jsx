...
function TableSelection() {
  ...
  const handleSelectTable = async (table) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/api/tables/${table._id}/status`, {
        status: "Occupied",
      });
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
                {tables.map((table) => (
                  <div
                    key={table._id}
                    className={`rounded-2xl border-2 p-5 shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-300 ${getTableStyle(
                      table.status
                    )}`}
                  >
                    ...
                    <select
                      value={table.status}
                      onChange={(e) =>
                        updateStatus(table._id, e.target.value)
                      }
                      className={`w-full px-3 py-2 rounded-lg text-sm font-semibold border mb-4
                        ${
                          table.status === "Available"
                            ? "bg-[#e0f5df] text-green-700 border-green-500"
                            : ""
                        }
                        ${
                          table.status === "Occupied"
                            ? "bg-[#ffe1e1] text-red-700 border-red-500"
                            : ""
                        }
                        ${
                          table.status === "Reserved"
                            ? "bg-[#fff0c9] text-yellow-700 border-yellow-500"
                            : ""
                        }
                      `}
                    >
